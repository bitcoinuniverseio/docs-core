...
| Action | What happens |
| --- | --- |
| **List** | You set a price and sign in your wallet. The listing opens after the market's checks pass |
...
Your asking price must cover the marketplace fee and the dust limit of the
payout. The market rejects a price that is too low, and the review screen shows
the fee and your net proceeds before you sign.
...
Checking asks the market, with a request signed by your wallet, whether the
earlier request applied. It never sends the action again. The lock clears once
the market confirms the request went through or that it never applied.
